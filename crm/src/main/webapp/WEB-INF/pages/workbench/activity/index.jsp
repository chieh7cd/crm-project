...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#createActivityBtn").click(function () {
			// 初始化工作，清空之前的表单数据 $("#createActivityForm").get(0)获取dom对象
			// 重置表单
            ...
            $.ajax({
                ...
                success:function (data) {
                    if(data.code == "1"){
                        ...
                        // 刷新市场活动列表, 显示第一页数据, 保持每页显示条数不变(暂时保留)
		...
		// 当市场活动页面加载完毕，查询所有数据的第一页以及所有数据的总条数
		queryActivityByConditionForPage(1, 10);

		// 给"查询"按钮绑定单击事件
		$("#queryActivityBtn").click(function () {
			// 查询所有符合条件数据的第一页以及所有符合数据的总条数
			queryActivityByConditionForPage(1, $("#demo_pag1").bs_pagination('getOption','rowsPerPage'));
		});
		...
		$("#deleteActivityBtn").click(function () {
			// 收集要删除的市场活动id主键值，获取所有被选中的checkbox的val值
		...
		$("#exportActivityXzBtn").click(function () {
			...
			if(checkedIds.length == 1){
				alert(checkedIds[0].value);
				window.location.href="workbench/activity/exportSelectedActivities.do?ids="+checkedIds[0].value;
			} else {
				var ids = "";
				$.each(checkedIds, function () { // ids=xxx&ids=xxx&ids=xxx&....
					ids += "ids="+this.value+"&";
				});
		...
		$("#importActivityBtn").click(function () {
			...
			// 表单验证，验证文件类型是不是.xls文件
			var suffix = activityFileName.substr(activityFileName.lastIndexOf(".") + 1).toLowerCase();//xls, XLS, Xls, xLs
			if(suffix != "xls"){
				alert("不支持的文件类型, 只支持.xls文件");
				return;
			}
	...
	queryActivityByConditionForPage = function(pageNo, pageSize){
		...
		$.ajax({
			...
			success:function (data) {
				...
				// 显示市场活动列表，遍历activityList，拼接所有行数据
				var htmlStr="";
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" role="form">
						<input type="hidden" id="edit-id"><%--隐藏域:不让用户看到，更新时把市场活动信息发到后台--%>
						<div class="form-group">
